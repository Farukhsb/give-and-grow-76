...
const Auth = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (forgotMode) {
      ...
      else toast({ title: "Check your email", description: "We've sent a password reset link." });
      setSubmitting(false);
      return;
    }
    if (isLogin) {
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md">
          <Card className="border-0 shadow-xl">
            <CardHeader className="text-center pb-2">
              ...
              <CardTitle className="font-serif text-2xl">{forgotMode ? "Reset Password" : isLogin ? "Welcome Back" : "Join CharityApp"}</CardTitle>
              <CardDescription>{forgotMode ? "Enter your email to receive a reset link" : isLogin ? "Sign in to continue making a difference" : "Create an account and start your giving journey"}</CardDescription>
            </CardHeader>
            <CardContent>
              {!forgotMode && (
                <>
                  {/* Google Sign In */}
                  <Button variant="outline" className="w-full mb-4 gap-2" onClick={handleGoogleSignIn}>
