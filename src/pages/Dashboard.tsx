...
const Dashboard = () => {
  ...
  const recommendations = useMemo(() => {
    ...
    return buildCharityRecommendations(charities, {
      cause: donorPreferences?.cause ?? "",
      helpMethod: donorPreferences?.help_method ?? "",
      location: donorPreferences?.location ?? "",
      urgency: "any",
    });
  }, [charities, donorPreferences]);
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select("cause, help_method, location")
      .eq("donor_id", user.id)
      .maybeSingle()
      .then(({ data, error }) => {
        if (error) {
          console.warn("Could not load donor preferences", error);
          setDonorPreferences(null);
          return;
        }

        setDonorPreferences((data ?? null) as DonorPreferences | null);
      });
  }, [user]);
